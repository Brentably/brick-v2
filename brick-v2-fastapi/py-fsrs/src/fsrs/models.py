...
class SchedulingCards:
    ...
    def __init__(self, card: Card, weight: float) -> None:
    ...
    def update_state(self, state: State) -> None:
        if state == State.New:
            ...
            self.good.state = State.Review
            self.easy.state = State.Review
            self.again.lapses += (1 * self.weight)
            
    def calculate_due(self, now: datetime, old_due: datetime, fsrs_calculated_due: datetime, weight: float) -> datetime:
        return fsrs_calculated_due
        # difference = (old_due - fsrs_calculated_due)
        # # If weight is 1, return fsrs_calculated_due directly
        # if weight == 1:
        #     # print(f"weight is 1, returning fsrs_calculated_due: {fsrs_calculated_due}")
        #     return fsrs_calculated_due
        # # Otherwise, move the due date closer to old_due based on weight
        # # weight of 0.2 means move 80% of the way back to old_due
        # # print(f"weight is {weight}, difference is {difference}")
        # weighted_difference = difference * (1 - weight)
        # return fsrs_calculated_due + weighted_difference

    def test_calculate_due(self):
        # Test case 1: Weight 0.2, FSRS date later
        test_old = datetime(2023, 11, 22, tzinfo=timezone.utc)
        test_fsrs = datetime(2023, 11, 27, tzinfo=timezone.utc)
        test_result = self.calculate_due(now=test_old, old_due=test_old, fsrs_calculated_due=test_fsrs, weight=0.2)
        expected = datetime(2023, 11, 23, tzinfo=timezone.utc)
        if abs((test_result - expected).total_seconds()) > 86400:  # Allow 1 day difference
            raise ValueError(
                f"Test case 1 failed: With weight=0.2 and FSRS date 5 days later,\n"
                f"Expected: {expected}\n"
                f"Got: {test_result}\n"
                f"The weighted due date should be closer to the old due date"
            )

        # Test case 2: Weight 1.0
        test_result = self.calculate_due(now=test_old, old_due=test_old, fsrs_calculated_due=test_fsrs, weight=1.0)
        expected = test_fsrs
        if test_result != expected:
            raise ValueError(
                f"Test case 2 failed: With weight=1.0,\n"
                f"Expected: {expected}\n"
                f"Got: {test_result}\n"
                f"The due date should exactly match the FSRS calculated date"
            )

        # Test case 3: Weight 0.2, FSRS date earlier
        test_fsrs_earlier = datetime(2023, 11, 12, tzinfo=timezone.utc)
        test_result = self.calculate_due(now=test_old, old_due=test_old, fsrs_calculated_due=test_fsrs_earlier, weight=0.2)
        expected = datetime(2023, 11, 20, tzinfo=timezone.utc)
        if abs((test_result - expected).total_seconds()) > 86400:
            raise ValueError(
                f"Test case 3 failed: With weight=0.2 and FSRS date 10 days earlier,\n"
                f"Expected: {expected}\n"
                f"Got: {test_result}\n"
                f"The weighted due date should be closer to the old due date"
            )
    ...
    def schedule(
        ...
        # print(f"self.again.due (check to make sure this matches previous due): {self.again.due}")
        # print(f"self.weight: {self.weight}")
        
        self.again.due = self.calculate_due(now, self.again.due, now + timedelta(minutes=5), self.weight) 
        if hard_interval > 0:
            self.hard.due = self.calculate_due(now, self.hard.due, now + timedelta(days=hard_interval), self.weight)
        else:
            self.hard.due = self.calculate_due(now, self.hard.due, now + timedelta(minutes=10), self.weight)
        self.good.due = self.calculate_due(now, self.good.due, now + timedelta(days=good_interval), self.weight)
        self.easy.due = self.calculate_due(now, self.easy.due, now + timedelta(days=easy_interval), self.weight)
